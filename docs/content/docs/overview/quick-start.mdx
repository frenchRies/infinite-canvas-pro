...
桌面版位于 `desktop/`，当前面向 Apple Silicon Mac，使用 Electron 打包。首次安装依赖后，在项目目录执行：
...
桌面版不自动迁移浏览器数据，也不自动启动 Canvas Agent。需要使用 Agent 时，另开终端运行 `npx -y @basketikun/canvas-agent`，再在画布中填写连接信息。
