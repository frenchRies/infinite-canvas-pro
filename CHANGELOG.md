...
+ [优化] 提示词搜索增加 300ms 防抖，减少连续输入时的重复查询。
+ [调整] KKAI `gpt-image-2` 按 URL-only 规范调用，支持显式质量与尺寸、逐张生成及多参考图编辑参数。
+ [调整] Studio 配色改为深夜石墨、雾青与琥珀点缀，统一浅色/深色页面、画布节点和工具栏的颜色层次。
+ [新增] Electron 桌面版新增可持久化的原版/工作室双 UI 风格切换，Studio 模式提供 macOS 创作工作区壳，并覆盖生成工作台与画布主题。
+ [修复] Studio 上下文栏项目改为可操作入口，支持在主页、画布、生成、提示词、资产和配置模块中定位或切换对应内容。
+ [新增] KKAI 渠道的 Grok Imagine 1.5 Video 支持专用图生视频请求、任务轮询和鉴权视频流下载。
+ [新增] KKAI 渠道接入 video-v2 多参考图、视频、音频模型，并提供 Seedance 系列显示名称。
+ [修复] 已保存的 KKAI 渠道配置自动补齐并迁移三个 Seedance 视频模型，保留原有模型和密钥设置。
+ [修复] Electron 桌面版生产服务改用固定本地端口，确保本地配置、画布和素材在重启后仍使用同一数据空间。
+ [调整] Electron 桌面版首次启动默认使用 KKAI 渠道和 gpt-image-2，网页端默认配置保持不变。
+ [新增] 增加面向 Apple Silicon Mac 的 Electron 桌面版打包与本地运行入口，保留现有网页和独立 Canvas Agent 使用方式。
...
+ [优化] 本地网页和 macOS App 启动时自动发现并连接本机 Canvas Agent，可直接使用 Codex 且无需重复填写连接地址和令牌。
